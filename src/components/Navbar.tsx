      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 justify-between">
          <div className="flex">
            ...
            {user && <div className="hidden sm:ml-6 sm:flex sm:space-x-8">
                <Link to="/dashboard" className={`inline-flex items-center border-b-2 px-1 pt-1 text-sm font-medium ${location.pathname === "/dashboard" ? "border-primary text-foreground" : "border-transparent text-muted-foreground hover:border-gray-300 hover:text-foreground"}`}>
                  Dashboard
                </Link>
                <Link to="/strategies" className={`inline-flex items-center border-b-2 px-1 pt-1 text-sm font-medium ${location.pathname.startsWith("/strategies") ? "border-primary text-foreground" : "border-transparent text-muted-foreground hover:border-gray-300 hover:text-foreground"}`}>
                  Strategies
                </Link>
                <Link to="/recommendations" className={`inline-flex items-center border-b-2 px-1 pt-1 text-sm font-medium ${location.pathname === "/recommendations" ? "border-primary text-foreground" : "border-transparent text-muted-foreground hover:border-gray-300 hover:text-foreground"}`}>
                  Recommendations
                </Link>
                <Link to="/backtest" className={`inline-flex items-center border-b-2 px-1 pt-1 text-sm font-medium ${location.pathname.startsWith("/backtest") ? "border-primary text-foreground" : "border-transparent text-muted-foreground hover:border-gray-300 hover:text-foreground"}`}>
                  Backtest
                </Link>
              </div>}
          </div>
      ...
      <div className="sm:hidden">
        <div className={`${isMenuOpen ? 'block' : 'hidden'} space-y-1 px-2 pb-3 pt-2`}>
          {user ? <>
              ...
              <Link to="/recommendations" className="block rounded-md px-3 py-2 text-base font-medium text-gray-700 hover:bg-gray-50 hover:text-gray-900" onClick={closeMobileMenu}>
                Recommendations
              </Link>
