...
export const RuleGroup = ({
  ...
  const handleInequalityChange = (updatedInequality: Inequality) => {
    ...
    const updatedInequalities = safeInequalities.map(inequality => inequality.id === updatedInequality.id ? updatedInequality : inequality);
  ...
  const orGroupDescription = title === "OR Group" ? `At least ${requiredConditions || 1} of ${effectiveInequalitiesCount} conditions must be met.` : description;
  ...
  return <div className={`mb-6 ${className || ''}`}>
      <div className={`${color === "blue" ? "bg-blue-50" : "bg-amber-50"} p-3 rounded-md mb-3`}>
        ...
        {title === "OR Group" && editable ? <div className="text-xs text-muted-foreground mb-2 flex items-center flex-wrap gap-2">
            <span>At least</span> 
            <div className="flex items-center">
              <Input type="number" min={1} max={maxRequiredConditions} value={conditionsCount} onChange={handleConditionsCountChange} className="w-16 h-6 px-2 py-0 inline-block mx-1 text-xs" />
              <TooltipProvider>
                <Tooltip>
      ...
      <div className="space-y-3">
        {safeInequalities && safeInequalities.length > 0 ? safeInequalities.map(inequality => <div key={`${title.toLowerCase().split(' ')[0]}-${inequality.id}`}>
              <RuleInequality inequality={inequality} editable={editable} onChange={handleInequalityChange} onDelete={editable ? () => handleInequalityDelete(inequality.id) : undefined} />
            </div>) : <div className="p-3 bg-gray-100 rounded text-center text-gray-500">
