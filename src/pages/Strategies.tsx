      ...
      <main className="flex-1 p-6">
        <div className="max-w-7xl mx-auto">
          ...
          {loading ? <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              {filteredStrategies.map(strategy => <StrategyCard key={strategy.id} name={strategy.name} description={strategy.description || "No description provided"} days={Math.floor((Date.now() - new Date(strategy.createdAt || Date.now()).getTime()) / (1000 * 60 * 60 * 24))} asset={strategy.targetAsset || strategy.market} status={strategy.isActive ? "active" : "inactive"} id={strategy.id} />)}
