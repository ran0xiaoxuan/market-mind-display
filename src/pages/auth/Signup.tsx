...
const Signup = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        toast.success("Account created! Please check your email (including spam folder) for verification.");
      }
    } catch (error: any) {
      console.error("Signup error:", error);
      toast.error(error.message || "Failed to create account");
  ...
  const handleResendConfirmation = async () => {
    ...
    setIsLoading(true);
    try {
      const { error } = await resendConfirmation(lastEmailSent);
      if (error) {
      ...
      <Card className="w-full max-w-md mx-auto">
        <CardHeader className="space-y-1">
          ...
          <CardDescription className="text-center">
            Enter your details to get started with StratAIge
          </CardDescription>
        </CardHeader>
            ...
            <div className="text-center space-y-4">
              <div className="p-4 bg-green-50 border border-green-200 rounded-md">
                ...
                <p className="text-sm text-green-700 mb-3">
                  We've sent a confirmation email to <strong>{lastEmailSent}</strong>
                </p>
